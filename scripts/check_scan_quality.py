...
def generate_rescan_script(skills: list[dict], output_path: Path,
    ...
    for skill in skills:
        ...
        lines.append(f"# {name} ({', '.join(skill['reasons'])})")
        lines.append(
            f'"$SCANNER" scan "{path}"'
            f" --use-behavioral --use-trigger --use-llm --enable-meta"
            f' --format json -o "$RESULTS_DIR/{stem}-scan.json"'
            f' 2>&1 | tee "$RESULTS_DIR/{stem}-scan.log"'
        )
        lines.append(
            f'"$SCANNER" scan "{path}"'
            f" --use-behavioral --use-trigger --use-llm --enable-meta"
            f' --format markdown -o "$RESULTS_DIR/{stem}-scan.md"'
        )
        lines.append("")

    lines.append(f"echo 'Done. Re-scanned {len(skills)} skills.'")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Scan Quality Report — {total} scans")
    print(f"{'='*60}\n")
